...
const getBooksInYear= async function (req, res) {
  ...
  let allBooks= await BookModel.find({ $or : [{year:year1}]})
  res.send({msg:allBooks})
}
//
const getParticularBooks= async function (req, res) {
    ...
    let year1 = req.query.year
    let allBooks = await BookModel.find({ $or : [{year:year1 }, {bookName: bookName1}]})
    res.send({msg:allBooks})
}
//
const getXINRBooks= async function (req, res) {
  let xinrBooks= await BookModel.find({"prices.indianPrice": {$in : ["500INR","200INR","100INR"]}})
  res.send(xinrBooks)
}
//
const getRandomnBooks= async function (req, res) {
...
module.exports.getRandomnBooks=getRandomnBooks
